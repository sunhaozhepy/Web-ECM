...
function start(e) {
    if (e.keyCode === 32 && situation === 'start') {
        ...
        ball_y = radius;
        life = canvas.width / 2;
        dx = 8 * parseInt(document.getElementById("ballspeed").value)
        dy = 8 * parseInt(document.getElementById("ballspeed").value)
        bar_x = 0;
        bar_width = canvas.width / (2 * parseInt(document.getElementById("barlength").value) + 2)
        situation = 'game';
        if (animation) {
...
function game() {
    ...
    const sentences = [
        'Noobily noobing NOOB',
        'RETARD',
        'IDIOT',
        'Why are you still playing? I wonder...',
        'STOP right now',
        'What a psycho',
        'ImPReSsIvE'
    ];
    ...
    if (life < 0) {
        ...
        ctx.textAlign = 'center'
        ctx.fillStyle = 'green';
        ctx.fillText("YOU FOOL! Press blankspace to retry!", canvas.width / 2, canvas.height / 2);
        situation = 'start';
        dx = 0;
        dy = 0;
    }
